...
if ( !A_IsCompiled )
	SetTimer, REL_ScriptReload, 2000
...
SYS_ExitHandler:
	...
	WinClose NumpadMouse_OkS.ahk - AutoHotkey
ExitApp
...
SUS_SuspendHandler:
	IfWinActive, A
	{
		...
		If ( (SUS_WinMinMax = 0) and (SUS_WinX = 0) and (SUS_WinY = 0) and (SUS_WinW = A_ScreenWidth) and (SUS_WinH = A_ScreenHeight) or ((SUS_WinClass = "#32770") or (SUS_WinClass = "MMCMainFrame") or (SUS_WinClass = "#32770") or (SUS_WinClass = "MMCMainFrame") or (SUS_WinClass = "TaskManagerWindow")) ) 
		{
			WinGetClass, SUS_WinClass, ahk_id %SUS_WinID%
...
NWD_WindowHandler:
		...
		Else
			If ( NWD_PermitClick or (!NWD_Dragging or ((NWD_MouseStartX = NWD_MouseX) and (NWD_MouseStartY = NWD_MouseY))) )
			{
				If ( !Timer("Cooldown") )
				{
					a = Timer("Cooldown,"L")
					MsgBox %a%
					;MouseClick, RIGHT, %NWD_MouseX%, %NWD_MouseX%, , , D
					;MouseClick, RIGHT, %NWD_MouseX%, %NWD_MouseY%, , , U				
					Click, RIGHT
				}
			}
	...
	Else
	{
		...
		If ( (NWD_MouseDeltaX >= 5) or (NWD_MouseDeltaY >= 5) )
		{
...
$!XButton1::
	...
	If ( TSM_RButtonState = "U" )
	{
		...
		Else If ( (TSM_CtrlState = "D") and (TSM_ShiftState = "D") and (TSM_AltState = "U") )
		{
			SleepTime = 200
		...
		Else If ( (TSM_CtrlState = "D") and (TSM_ShiftState = "U") and (TSM_AltState = "D") )
		{
			Send, {Tab}
		}
	...
	Else If ( TSM_RButtonState = "D" )
	{
		IfWinActive, A
		{
			...
			If ( TSM_WinClass != "Progman" )
			{
				...
				Else If ( TSM_CtrlState = "D" )
				{
					If ( TSM_WinClass = "CabinetWClass" )
					{
						Send, {LAlt}{Up}
					}
				}
...
$!XButton2::
	...
	If ( MAW_RButtonState = "U" )
	{
		...
		Else If ( (MAW_CtrlState = "D") and (MAW_ShiftState = "D") and (MAW_AltState = "U") )
		{
		...
		Else If ( (MAW_CtrlState = "D") and (MAW_ShiftState = "U") and (MAW_AltState = "D") )
		{
			Send, {Shift}+{Tab}
		}
...
!#u::			; rerun this script
{
 	Run, "D:\Programs\AutoHotkey\MyLib\Other\NiftyWindows\NiftyWindows.ahk"
...
#If GetKeyState("NumLock", "T") ; True if NumLock is ON, false otherwise.
{
	...
	^Numpad7::
	{
		...
		InputBox, UserInput, Mouse Sensitive, Please enter a number from 1 to 20., , 240, 140
		Transform, UserInput, Ceil, UserInput
		If ErrorLevel
			Return
		Else
			MsgBox, You entered %UserInput%. Previous value is %OrigMouseSpeed%.
		DllCall("SystemParametersInfo", UInt, SPI_SETMOUSESPEED, UInt, 0, Ptr, UserInput, UInt, 0)
	...
	^Numpad9::
					...
					MsgBox, 3,, WinCompose is running. Would you like to %FutureWinComposeAction% WinCompose?, 10
					IfMsgBox Yes
					...
					IfMsgBox No
					{
						...
						IfMsgBox No
						{
							MsgBox, 259,, Would you like to (re)run WinCompose?
							IfMsgBox Yes
								Run, "D:\Programs\WinCompose-NoInstall-0.9.0\wincompose.exe"
							IfMsgBox No
							{
...
`::
If WinActive("ahk_class explorer.exe")
...
>^CapsLock::
{
	...
	MsgBox %l%`n`nUk_U2 := 0xF0C20422`nUk_Ex := 0xF0A80422`nRu_Uk:= 0x4192000`nRu:= 0x4190419`nEn_ExLa:= 0xF0C10409`nEn_US:= 0x04090409
En_Co	:= 0xF0CE0409
}
Return
